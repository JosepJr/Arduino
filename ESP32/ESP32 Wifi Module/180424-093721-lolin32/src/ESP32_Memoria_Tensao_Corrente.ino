  ...
  */
   ...
   */
   ...
   for (int i = 0; i < MAX; i++) {
     ...
     //deste modo eu simulo a leitura de 556x3 e consigoo tempo de 17ms.
     x = analogRead(33);
   }
   Tempo2 = millis();
   ...
   Tempo1 = millis();
   for (int i = 0; i < MAX; i++) {
     VTensao[i] = (((VTensao[i]*(3.3))/4095)-(1.65));
     VCorrente[i] = (((VCorrente[i]*(3.3))/4095)-(1.44)); //erro de -0.21
   }
   Tempo2 = millis();
   ...
   for(int i = 0; i < MAX; i++){
     VMedia[i] = VTensao[i] * VCorrente[i];
   }

   //PotenciaMedia = Variavel de Potencia Media
   // Potencia Instantanea Acomulada
   float PotenciaMedia = 0;
   for(int i = 0; i < MAX; i++){
          PotenciaMedia = PotenciaMedia + VMedia[i];
   }
   //Calculando Potencia Media
   PotenciaMedia = PotenciaMedia / MAX;
